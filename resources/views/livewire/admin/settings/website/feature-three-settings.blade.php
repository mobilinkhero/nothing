...
<div x-data="formComponent()">
    ...
    <div class="flex flex-wrap lg:flex-nowrap gap-4">
        ...
        <div class="flex-1 space-y-6">
            <form x-on:submit.prevent="validateAndSubmit" class="space-y-8"  x-data="{ 'feature_three_enabled': @entangle('feature_three_enabled') }">
                <x-card class="rounded-lg">
                    <x-slot:header class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div>
                            <x-settings-heading>
                                {{ t('feature_three') }}
                            </x-settings-heading>
                        ...
                        <div class="flex justify-end items-center">
                            <x-toggle id="feature_three_toggle" name="feature_three_toggle" :value="$feature_three_enabled"
                                x-on:toggle-changed="feature_three_enabled = $event.detail.value" />
                        </div>
                    </x-slot:header>
                    <x-slot:content class="space-y-4">
                        <!-- Feature Title -->
                        <div>
                            <x-label for="feature_title_three" :value="t('feature_title')" />
                            <x-input id="feature_title_three" type="text" class="mt-1 block w-full"
                                wire:model="feature_title_three" autocomplete="off" />
                            <x-input-error for="feature_title_three" class="mt-2" />
                        </div>

                        <!--  Feature Subtitle -->
                        <div>
                            <x-label for="feature_subtitle_three" :value="t('feature_subtitle')" />
                            <x-input id="feature_subtitle_three" type="text" class="mt-1 block w-full"
                                wire:model="feature_subtitle_three" autocomplete="off" />
                            <x-input-error for="feature_subtitle_three" class="mt-2" />
                        </div>

                        <!--  Feature Description -->
                        <div>
                            <x-label for="feature_description_three" :value="t('feature_description')" />
                            <x-textarea id="feature_description_three" rows="4" class="mt-1 block w-full"
                                wire:model="feature_description_three"></x-textarea>
                            <x-input-error for="feature_description_three" class="mt-2" />
                        </div>
                        ...
                        <div>
                            ...
                            <div class="space-y-2">
                                @foreach ($feature_list_three as $index => $listItem)
                                <div class="flex items-center gap-4">
                                    <x-input type="text" class="block w-full"
                                        wire:model="feature_list_three.{{ $index }}" autocomplete="off" />
                        ...
                        <!--  Feature Image -->
                        <div class="w-full pt-4 upload-section" x-data="fileUploader('feature_image_three')">
                            <x-label for="feature_image_three" :value="t('feature_image')" class="mb-2" />
                            ...
                            <div class="relative p-6 border-2 border-dashed rounded-lg cursor-pointer hover:border-info-500 transition duration-300"
                                ...
                                <template x-if="preview">
                                    <div class="relative inline-block">
                                        ...
                                        <button type="button"
                                            ...
                                            x-on:click.stop="confirmDelete($event, () => { clearImage(); $wire.set('feature_image_three', ''); })">
                                ...
                                <template x-if="!preview && '{{ $featureSettings['theme.feature_image_three'] }}'">
                                    <div class="relative inline-block">
                                        <img src="{{ asset('storage/' . $featureSettings['theme.feature_image_three']) }}"
                                ...
                                <input x-ref="imagePathInput" type="file" class="hidden" accept=".png,.jpg,.jpeg"
                                    x-on:change="handleFileChange" wire:model="feature_image_three"
                            ...
                            <!-- Error Message -->
                            @if ($errors->any())
                            <x-input-error for="feature_image_three" class="mt-2" />
                            @else
