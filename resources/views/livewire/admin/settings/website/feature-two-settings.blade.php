    ...
    <div class="flex flex-wrap lg:flex-nowrap gap-4">
        ...
        <div class="flex-1 space-y-6">
            <form x-on:submit.prevent="validateAndSubmit" class="space-y-8"
                x-data="{ 'feature_two_enabled': @entangle('feature_two_enabled') }">
                <x-card class="rounded-lg">
                    <x-slot:header class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        ...
                        <div class="flex justify-end items-center">
                            <x-toggle id="feature_two_toggle" name="feature_two_toggle" :value="$feature_two_enabled"
                                x-on:toggle-changed="feature_two_enabled = $event.detail.value" />
                    ...
                    <x-slot:content class="space-y-4">
                        <!-- Feature Title -->
                        <div>
                            <x-label for="feature_title_two" :value="t('feature_title')" />
                            <x-input id="feature_title_two" type="text" class="mt-1 block w-full"
                                wire:model="feature_title_two" autocomplete="off" />
                            <x-input-error for="feature_title_two" class="mt-2" />
                        </div>

                        <!--  Feature Subtitle -->
                        <div>
                            <x-label for="feature_subtitle_two" :value="t('feature_subtitle')" />
                            <x-input id="feature_subtitle_two" type="text" class="mt-1 block w-full"
                                wire:model="feature_subtitle_two" autocomplete="off" />
                            <x-input-error for="feature_subtitle_two" class="mt-2" />
                        </div>

                        <!--  Feature Description -->
                        <div>
                            <x-label for="feature_description_two" :value="t('feature_description')" />
                            <x-textarea id="feature_description_two" rows="4" class="mt-1 block w-full"
                                wire:model="feature_description_two"></x-textarea>
                            <x-input-error for="feature_description_two" class="mt-2" />
                        </div>
                        ...
                        <div>
                            ...
                            <div class="space-y-2">
                                @foreach ($feature_list_two as $index => $listItem)
                                <div class="flex items-center gap-4">
                                    <x-input type="text" class="block w-full" wire:model="feature_list_two.{{ $index }}"
                                        autocomplete="off" />
                        ...
                        <!--  Feature Image -->
                        <div class="w-full pt-4 upload-section" x-data="fileUploader('feature_image_two')">
                            <x-label for="feature_image_two" :value="t('feature_image')" class="mb-2" />
                            ...
                            <div class="relative p-6 border-2 border-dashed rounded-lg cursor-pointer hover:border-info-500 transition duration-300"
                                ...
                                <template x-if="preview">
                                    <div class="relative inline-block">
                                        ...
                                        <button type="button"
                                            ...
                                            x-on:click.stop="confirmDelete($event, () => { clearImage(); $wire.set('feature_image_two', ''); })">
                                ...
                                <template x-if="!preview && '{{ $featureSettings['theme.feature_image_two'] }}'">
                                    <div class="relative inline-block">
                                        <img src="{{ asset('storage/' . $featureSettings['theme.feature_image_two']) }}"
                                ...
                                <input x-ref="imagePathInput" type="file" class="hidden" accept=".png,.jpg,.jpeg"
                                    x-on:change="handleFileChange" wire:model="feature_image_two"
                            ...
                            <!-- Error Message -->
                            @if ($errors->any())
                            <x-input-error for="feature_image_two" class="mt-2" />
                            @else
